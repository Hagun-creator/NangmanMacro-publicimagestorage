...
import json,struct,re,shutil,zlib,binascii,os,urllib.request,urllib.parse
...
# IMPORTANT:
# The old pre-seeded Korean preview images in data/verified.json and data/verified/*.png
# are intentionally ignored. From this reset onward, ONLY GitHub Issues carrying the
# '검증완료' label are treated as verified Korean images.
...
def png_bytes(w,h,bits):
    ...
    raw=b''.join(rows)
    def chunk(t,b):
        return struct.pack('>I',len(b))+t+b+struct.pack('>I',binascii.crc32(t+b)&0xffffffff)
    return (
        b'\x89PNG\r\n\x1a\n'
...
def github_json(url):
    ...
    headers={
        ...
        'User-Agent':'nangman-image-browser'
    }
    if token:
...
def body_field(body,label):
    ...
    if not m:
        return ''
    v=m.group(1).strip()
    if v in ('_No response_','No response'):
        return ''
...
def load_verified_issues(valid_keys):
    ...
    for page in range(1,11):
        ...
        try:
            issues=github_json(url)
        except Exception as e:
            print('verified issue sync skipped:',e)
            break
        if not issues:
            break
        for issue in issues:
            ...
            if key not in valid_keys:
                print('verified issue key not found:',issue.get('number'),key)
                continue
            url_img=attachment_url(issue.get('body',''))
            if not url_img:
                print('verified issue has no image:',issue.get('number'),key)
                continue
            old=found.get(key)
            if old and (old.get('updated_at') or '') >= (issue.get('updated_at') or ''):
                continue
            found[key]=issue
        if len(issues)<100:
            break
    ...
    for key,issue in found.items():
        try:
            ...
            label_ko=(
                body_field(issue.get('body',''),'화면에 보이는 한국어 이름')
                or key.split('/',1)[-1]
            )
            result[key]={
                'label':label_ko,
...
d=(DATA/'templates.bin').read_bytes()
pos=4
ver=struct.unpack('>I',d[pos:pos+4])[0]
pos+=4
cnt=struct.unpack('>I',d[pos:pos+4])[0]
pos+=4
...
for i in range(cnt):
    ...
    if vr_issue:
        vr={
            'label':vr_issue['label'],
            'note':vr_issue['note'],
            'png':vr_issue['png'],
            'issue_number':vr_issue['issue_number'],
            'issue_url':vr_issue['issue_url']
        }
...
for name in sorted({x['category'] for x in records}):
    ...
    uniq={x['canonical_id'] for x in arr}
    done={x['canonical_id'] for x in arr if x['verified']}
    cats.append({
        'name':name,
        'raw_count':len(arr),
        'unique_count':len(uniq),
        'duplicate_count':len(arr)-len(uniq),
        'verified_count':len(done),
        'usage':arr[0].get('category_usage','')
    })
...
manifest={
    ...
    'categories':cats,
    'records':records,
    'issue_verified_count':len(issue_verified),
    'verified_source':'github_issues_only'
}
(OUT/'generated'/'manifest.json').write_text(
